...
#define RAY_SUBSAMPLES 20
#define MAX_ROUGHNESS 0.8f
#define MAX_RAYS 1
#define DEPTH_TOLERANCE 2.0f
#define STEP_DISTANCE_FACTOR 0.2f
#define MAX_JITTER 0.5f
...
#else
vec4 SSRTrace(in vec3 position, in vec2 texCoords, in sampler2D diffusemap, in sampler2D depthmap, in sampler2D normalmap, in sampler2D metalroughnessmap, in sampler2D specularcolormap)
#endif
{
#ifndef GBUFFER_MSAA
    int samp = 0;
...
#ifdef GBUFFER_MSAA
    vec2 ntexsize = textureSize(normalmapMS);
#else
    vec2 ntexsize = textureSize(normalmap, 0);
#endif
    ivec2 icoord = ivec2((gl_FragCoord.xy / BufferSize) * ntexsize);
#ifdef GBUFFER_MSAA    
    vec3 normal = texelFetch(normalmapMS, icoord, samp).rgb;
#else
    vec3 normal = texelFetch(normalmap, icoord, 0).rgb;
#endif
	
    if (normal.x == 0.0f and normal.y == 0.0f and normal.z == 0.0f) return vec4(0);

    //if (normal.y < 0.707) return vec4(0);

    normal = normalize(normal);
    position += normal * 0.01f;

    vec3 metallicroughness = texelFetch(metalroughnessmap, icoord, samp).rgb;
    ...
    //for (int p = 0; p < raycount; ++p)
    {
        ...
        normal = startnormal;
        stepsize = max(MIN_STEP_SIZE, position.z * STEP_DISTANCE_FACTOR);// * max(0.5f, 1.0f - abs(normal.z));
//#if RAY_SAMPLES        
        //if (p > 0 && roughness > 0.0f)
        //{
        //    vec3 jitter = hash(vec3(gl_FragCoord.xy, gl_FragCoord.x * gl_FragCoord.y) * fragposition * float(2.0f)) - 0.5f;
        //    normal += jitter * roughness * MAX_JITTER;
        //}
//#endif
/*#if RAY_SAMPLES > 1
        //if (p > 0)
        ...
        position += normal * stepsize;// * random(gl_FragCoord.xy + position.zy * position.x);

        for (int n = 0; n < MAX_STEPS; ++n)
        {
            stepsize = max(MIN_STEP_SIZE, position.z * STEP_DISTANCE_FACTOR);
            ...
            if (z < screencoord.z)
            {
                ...
                {
                    screencoord = CameraPositionToScreenCoord(position);
                    //glposition = CameraProjectionMatrix * vec4(position, 1.0f);
                    //glposition.z = (glposition.z + glposition.w) * 0.5f;
                    //screencoord = glposition.xyz;
                    //screencoord.xy = screencoord.xy / glposition.w * 0.5 + 0.5;                
                    
                    if (screencoord.z < CameraRange.x or screencoord.z > CameraRange.y or screencoord.x < 0.0 or screencoord.x > 1.0 or screencoord.y < 0.0 or screencoord.y > 1.0) return vec4(0);
                ...
                //return vec4(1,0,0,1);
                //if (screencoord.z < z)// * min(cameranormal.z * stepsize * 6.0f, 1.0f))
                {
                    float alpha = 1.0f;
                    ...
                    //Distance fade
                    float diff = abs(z - screencoord.z);
                    alpha = 1.0;//2.0f - (diff - 1.0f);//1.0f - (diff - 0.5) / 1.0f;               
                    ...
                    alpha = 1.0;
                    {
                        ...
                        vec4 c = textureLod(diffusemap, screencoord.xy, 0);
                        c.rgb *= alpha;
                        c.a = alpha;
                        raysample += c;
                    }
