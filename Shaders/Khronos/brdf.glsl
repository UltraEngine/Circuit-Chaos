#ifndef _KHRONOS_BRDF
#define _KHRONOS_BRDF
...
vec3 Schlick_to_F0(vec3 f, float VdotH) {
    return Schlick_to_F0(f, vec3(1.0f), VdotH);
}

float Schlick_to_F0(float f, float VdotH) {
    return Schlick_to_F0(f, 1.0f, VdotH);
}
...
vec3 BRDF_specularGGX(vec3 f0, vec3 f90, float alphaRoughness, float specularWeight, float VdotH, float NdotL, float NdotV, float NdotH)
{
    ...
    NdotV = clamp(NdotV, 0.0f, 0.98f);
    NdotL = clamp(NdotL, 0.0f, 0.98f);
